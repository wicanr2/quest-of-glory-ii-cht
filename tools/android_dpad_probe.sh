...
LOG "== 5) 雙擊 5 Days a Stranger 進遊戲(ScummVM 雙擊遊戲列 = start) =="
# 上一輪 tap 160,115 沒選中(x 偏右,5days 文字只到 x≈150)、Start 也沒觸發 → 還停 launcher。
# 改點文字中段 x=90 + 雙擊直接 start。launcher 是 menu context、touch_mode_menus 預設
# mouse(direct),雙擊有效。
adb shell input tap 90 113; sleep 1; SHOT shot_05_selected.png
adb shell input tap 90 113; sleep 3
SHOT shot_06_starting.png

LOG "== 5b) 備援:若雙擊沒進,補點右下角 Start 紅鈕 =="
adb shell input tap 250 588; sleep 3
