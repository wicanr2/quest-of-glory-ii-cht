...
| 全文字靜態抽取 | ✅ 全域訊息 + 各房間 room script，含 NPC 對話（早期漏抽 room script，補上後對話才完整） |
...
一句話收束這張表：能讀的、能聽的，都中文了。旁白描述（2352 句）加上 NPC 對話（6791 句）合計逾九千句繁中、引擎端到端吐得出方塊字、桌面三平台一鍵就玩——剩下卡住的幾格（角色創建的引擎 GUI 標籤、Android 打包的 Oboe 連結）都不擋你坐進夏皮爾的市集聽人說書。誠實標 🚧／⛔ 是怕你期待落空，不是這座城還沒蓋好。
...
劇情往下走，對話也一路都是中文 —— 客棧裡的長者向你道謝：「在夏皮爾(Shapeir)，你永遠有個落腳處，有美食可享用。沙明、舍瑪和我，真不知該如何謝你幫我們尋回被偷走的寶藏。」連 NPC 的名字都譯好了，九千多句旁白與對話是一句句校過的，不是機器轉的：

![實機中文 NPC 劇情對話：客棧長者道謝](docs/screenshot-gameplay-dialog2.png)
...
而封印那四隻元素，每一隻都有自己的門道，當年沒有 GameFAQ、沒有 wiki，全靠三大誌的攻略一步步帶你過。火元素你得把燈放在地上，等牠靠近了用水潑小，再收進燈裡；風元素打不到、抓不著，你得用懸浮術（Levitate）浮得比牠高，憋著氣一動也不動地等牠飄近——「用力等」，攻略是這麼寫的；土元素隨機冒出來，最賤，你得在街道口兩個畫面之間來回踱步把出現率拉高，一現身就得搶在牠揍你一拳逃跑前打中；水元素則要在噴水池前擺好水袋，用風箱（Bellows）把牠吹進去。光是這四隻，就夠你在夏皮爾的巷弄裡跑上半天。

夏皮爾還有許多只有玩過的人才懂的小聰明。跟索魯斯（Saurus）攤的老闆殺價，老手都知道別急著用議價技能，就一路說「不」，拗到對方只剩五個第納爾才點頭——再把那頭蜥蜴坐騎連騎個五趟，殺價技能練上去，低價就買得下牠了。午夜的潛行更是這一代的招牌：摸進別人家裡，先把油倒在衣櫃門栓上免得開門時「咿呀」一聲驚動了人，再翻出銀茶具與翡翠碗，最後掀開地板下那道暗門。當年《軟體世界》的編輯講到買油那段還不忘補刀——被怪物追殺時可以「腳底抹油」溜之大吉，「開玩笑的，說明書可沒提到這種用途」（《軟體世界》第026/027期，1991）。那種一邊帶你破關、一邊自己先笑場的親切人聲，就是那個年代攻略陪你玩的方式。

而它真正的浪漫，是讓你**把一代的角色匯入二代**：你在史畢柏格存下的那個英雄，帶著他的力量、法術與榮譽，繼續走進沙漠。盜賊、法師、戰士三種出身一路成長，戰士若榮譽夠高、心夠正，能在這一代首度走上**聖騎士（Paladin）**之路——這條線，是整個系列最動人的設計之一。一路撐到拉希爾的寶物室，老玩家都記得那盞撿到的神燈、跳出來給你三個願望的精靈，以及最終戰那記反敗為勝的妙著：身上罩著「反轉」之力（Reversal），阿德·阿維斯一道原力箭打過來，原封不動反彈回去，把他自己轟下了高台。那一刻，前面所有的取捨與養成，全在這一擊裡結了帳。
...
到了二代，這套設計被推到了極致。當年《軟體世界》的攻略乾脆附上三張計分表——「戰士可得之分數」「魔法師可得之分數」，還有一張「結束時給聖騎士的加分」——攤開來你才看懂，這根本不是同一款遊戲玩三遍，而是三條截然不同的路。戰士靠的是硬碰硬：破門而入、與哈維因（Khaveen）正面決鬥；盜賊吃的是午夜那套，撬鎖、潛行、把茶具翡翠碗一件件摸走；法師則是一道道法術解題，連最後那場魔法測驗都要照著順序施展取物術（Fetch）、原力箭（Force Bolt）、火鏢（Flame Dart）才闖得過。同一個謎題，你選什麼出身，眼前看到的解法就完全是另一回事——這份「為你的選擇量身打造後果」的野心，是 1990 年角色扮演裡少見的奢侈。
...
這套螢幕 D-pad 的四份文件,由「怎麼開」到「驗到哪」:

| 文件 | 講什麼 |
|---|---|
| [`docs/android-dpad-virtual-mouse.md`](docs/android-dpad-virtual-mouse.md) | **機制**:三種觸控模式、`VirtualMouse` + keymapper、config key、本專案三層啟用方案 |
| [`docs/android-dpad-design.svg`](docs/android-dpad-design.svg) | **設計圖**:上面那張「D-pad → 游標」控制鏈一圖看懂 |
| [`docs/ci-android-dpad-test-plan.md`](docs/ci-android-dpad-test-plan.md) | **CI 驗證計畫**:用 GitHub Actions 模擬器自動驗 D-pad（**不涉版權**、免費 AGS 遊戲當素材）的素材選定、workflow 與踩雷表 |
| [`docs/android-dpad-outcome.md`](docs/android-dpad-outcome.md) | **成果記錄**:五輪 CI 迭代的根因、已實證到 launcher（控制器圖示＋切 Gamepad＋游標被觸控移動，有截圖）、AGS 遊戲內推 D-pad 仍 TODO（誠實版） |
| [`docs/android-touch-controls.md`](docs/android-touch-controls.md) | **真機操作經驗（實測定案）**:左/右/中鍵＝單指「點按住的時間長短」(快點=左、按住0.5~1.5秒=右、>1.5秒=中)、點擊類冒險選 Direct Mouse、推薦 `scummvm.ini`、鍵盤/片頭動畫已知問題、以及各平台移植踩雷(libmad 缺失/configure 參數順序/oboe ABI/mpeg 解碼) |

> 真機實測（Galaxy S23 Ultra / Android 16）定案:螢幕 D-pad 在遊戲場景沒穩定畫出,**點擊類冒險改用 `Direct Mouse` 模式 + 上表「按住時長＝滑鼠鍵」最順手**。完整操作與移植踩雷見上表最後一份文件。
